...
def cleanNoise(edged_image):
    ...
    for x in range(rows):
        for y in range(columns):
            travelPath(x, y, viewedPixelCoordinates,
                       validPixelCoordinates, mapObjects, edged_image)
...
def travelPath(xIndex, yIndex, viewedPixelCoordinates, validPixelCoordinates, mapObjects, edged_image):
    ...
    northEast = (xIndex - 1 if xIndex - 1 >= 0 else 0, yIndex +
                 1 if yIndex + 1 < columns else columns - 1)
    northWest = (xIndex - 1 if xIndex - 1 >= 0 else 0,
                 yIndex - 1 if yIndex - 1 >= 0 else 0)
    south = (xIndex + 1 if xIndex + 1 < rows else rows - 1, yIndex)
    southEast = (xIndex + 1 if xIndex + 1 < rows else rows - 1,
                 yIndex + 1 if yIndex + 1 < columns else columns - 1)
    southWest = (xIndex + 1 if xIndex + 1 < rows else rows -
                 1, yIndex - 1 if yIndex - 1 >= 0 else 0)
        ...
        if edged_image[center[1], center[0]] == 255:
            ...
            for direction in directions:
                if edged_image[direction[1], direction[0]] == 255:
                    pathExists = True
                    travelPath(direction[0], direction[1], viewedPixelCoordinates,
                               validPixelCoordinates, mapObjects, edged_image)
...
def createMesh(mapObjects):
    ...
    for index, vertice in enumerate(vertices):
        if vertice[2] == 0:
            faces.append([index, (index + 1) % len(vertices),
                         vertices.index((vertice[0], vertice[1], 30))])
            #faces.append([index, vertices.index((vertice[0], vertice[1], 30)), (vertices.index((vertice[0], vertice[1], 30)) + 1) % len(vertices)])
        else:
            faces.append([(index - 1) % len(vertices),
                         vertices.index((vertice[0], vertice[1], 0)), index])
...
def main():
    ...
    showImages(loaded_image=loaded_image, edged_image=cv2.imread(
        tempFilePath), clear_edged_image=cv2.imread(tempCleanFilePath))
